...
function M.draw_into(png_path, win)
  ...
  if provider ~= "images.nvim" then
    -- Only images.nvim exposes drawing into an arbitrary existing window
    -- (`browse.draw_in_window`); snacks/image.nvim need a buffer they own,
    -- which a borrowed hover window is not.
    return nil
  end
