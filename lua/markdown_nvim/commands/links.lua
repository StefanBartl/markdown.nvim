---@module 'markdown_nvim.commands.links'
...
local function do_create(argv)
  require("markdown_nvim.commands.markdown_links").run(argv)
end
...
local function do_show(argv)
  ...
  local picker = require("markdown_nvim.util.picker")
  picker.select(links, {
    prompt  = string.format("Markdown links (%d)", #links),
    format  = format_item,
    backend = (cfg.links and cfg.links.picker) or "hover_select",
  }, function(lk)
    require("markdown_nvim.handler").open_target(lk.target)
  end)
end
