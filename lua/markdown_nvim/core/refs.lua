---@module 'markdown_nvim.core.refs'
...
local notify = require("markdown_nvim.util.notify").create("[markdown_nvim.core.refs]")
local slug   = require("markdown_nvim.core.slug")
...
local NS = api.nvim_create_namespace("markdown_nvim_refs")
...
function M.reconcile(bufnr, opts)
  ...
  if conf.update_toc ~= false then
    ...
    if has_toc then
      pcall(function() require("markdown_nvim.commands.toc").update(header, {}) end)
    end
  end
