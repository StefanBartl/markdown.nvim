...
return function(H)
  ...
  eq(
    marker_default.fg,
    tonumber("6A9955", 16),
    "marker_fg defaults to the fixed VS Code-style green"
  )
  ...
  ok(
    type(vim.api.nvim_get_hl(0, { name = MARKER }).fg) == "number",
    "marker fg resolved without explicit config"
  )
  ok(
    type(vim.api.nvim_get_hl(0, { name = TEXT }).fg) == "number",
    "text fg resolved without explicit config"
  )
  ...
  eq(
    text0[3],
    marker0[4].end_col,
    "highlight_line: text extmark starts where the marker extmark ends"
  )
  ...
  eq(
    #marks0_after,
    0,
    "highlight_line: removing the leading '>' clears the stale marker/text extmarks"
  )
end
