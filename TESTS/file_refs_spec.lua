...
return function(H)
  ...
  local run_ok2, err2 = pcall(function()
    ...
    local function write(rel, lines)
      local fh = io.open(root2 .. "/" .. rel, "w"); ok(fh ~= nil, "fixture2 write: " .. rel)
      fh:write(table.concat(lines, "\n")); fh:close()
    end
    write("docs/target.md", { "# Target" })
    ...
    eq(#refs, 4, "find_references: finds all 4 link styles incl. the cwd-relative one (the reported bug)")

    -- Index refs by containing file for targeted retarget assertions.
    local by = {}
    for _, r in ipairs(refs) do by[vim.fn.fnamemodify(r.file, ":t")] = r end

    -- retarget each to a rename: docs/target.md -> docs/renamed.md
    local new_abs = root2 .. "/docs/renamed.md"
    eq(file_refs.retarget(by["a_same.md"], new_abs), "renamed.md",
      "retarget: plain same-dir keeps plain form")
    eq(file_refs.retarget(by["a_dot.md"], new_abs), "./renamed.md",
      "retarget: './'-prefixed keeps the './' prefix")
    eq(file_refs.retarget(by["a_cwd.md"], new_abs), "docs/renamed.md",
      "retarget: cwd-relative stays cwd-relative")
    eq(file_refs.retarget(by["a_up.md"], new_abs), "../renamed.md",
      "retarget: '..'-climbing base recomputes correctly")

    -- Async returns the same set as sync.
    local async_refs, done = nil, false
    file_refs.find_references_async(target, { root = root2 }, function(r) async_refs = r; done = true end)
